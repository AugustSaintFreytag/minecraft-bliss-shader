...
void main() {
	...
	if (gl_FragCoord.x >= SKY_CLOUD_ATLAS_OFFSET_X && gl_FragCoord.x < SKY_CLOUD_ATLAS_OFFSET_X + SKY_ATLAS_SIZE + 1.0 && gl_FragCoord.y >= SKY_CLOUD_ATLAS_OFFSET_Y && gl_FragCoord.y < SKY_CLOUD_ATLAS_OFFSET_Y + SKY_ATLAS_SIZE + 1) {
		...
		vec3 skyAtlasFogAmbient = skyGroundCol / 30.0;
		vec3 skyAtlasFogAmbientAverage = skyColBase;

		vec4 volumetricClouds = GetVolumetricClouds(viewPos, vec2(noise, 1.0 - noise), WsunVec_local, suncol * volumetricFogLightBoost, skyAtlasFogAmbient, cloudPlaneDistance);
		vec4 volumetricFog = GetVolumetricFog(viewPos, vec2(noise, 1.0 - noise), WsunVec_local, 0.0, suncol * volumetricFogLightBoost, skyAtlasFogAmbient, skyAtlasFogAmbientAverage, cloudPlaneDistance, true);
