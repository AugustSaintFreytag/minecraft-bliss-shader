...
vec3 lighten(vec3 color, float boost, float pivot) {
	float luminance = dot(color, lumCoeff);
	float attenuation = pivot - pow(clamp(luminance, 0.0, 1.0), 3);   
...
vec3 cartToSphere(vec2 coord) {
    // coord.y=0 -> lat=hPI, coord.y=0.5 -> lat=0, coord.y=1 -> lat=-hPI
    float lon = coord.x * TAU;
    float lat = (0.5 - coord.y) * PI;

    vec2 sc_lon = sincos(lon);
    float cos_lat = cos(lat);
    
    return vec3(sc_lon.x * cos_lat, sin(lat), sc_lon.y * cos_lat);
}
...
vec2 sphereToCart(vec3 dir) {
    ...
    return vec2(lon * rTAU + 0.5, 0.5 - lat * rPI);
}
